...
class FeedForwardNNLM(nn.Module):
    ...
    def sample(model,vocab,x,steps,temperature=1.0):
        ...
        for k in range(steps):
            x_cond = x[:, -model.context_size_size:]
            logits = model(x_cond)
    ...
    def sample_ffnnlm(context,steps=10,model_path='ffnnlm.model',temperature=1.0):
        # 判断是否有可用的GPU
        device = torch.device("cuda" if torch.cuda.is_available() else 'cpu')
        # 加载词汇表和模型参数
        vocab,model = load_pretrained(model_path)
        context = context.lower().split()
        context = model.context_size * [BOS_TOKEN] + context
    ...
    sample_ffnnlm("",200,"ffnnlm.model",0.8)